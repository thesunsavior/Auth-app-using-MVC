...
class Model
{
    ...
    public function Update_validate(array $attrArray)
    {
        foreach ($this->rules() as $attribute => $rules) {
            if (!in_array($attribute, $attrArray))
                continue;
            $value = $this->{$attribute};
            foreach ($rules as $rule) {
                $ruleName = $rule;
                ...
                if ($ruleName === self::RULE_UNIQUE) {
                    ...
                    $statement = $db->prepare("SELECT * FROM $tableName WHERE $uniqueAttr = :$uniqueAttr AND NOT id = $pk");
                    $statement->bindValue(":$uniqueAttr", $value);
                    $statement->execute();
                    $record = $statement->fetchObject();
                    if ($record) {
